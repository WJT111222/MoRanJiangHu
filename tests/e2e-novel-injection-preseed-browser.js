(function () {
  const E2E_PLAYER_NAME = '红楼遗梦';
  const E2E_PARTNER_NAME = '青霜秘档';
  const E2E_CNB_USERNAME = '小号';
  ...
  const putSetting = (db, key, value) => new Promise((resolve, reject) => {
    ...
    tx.objectStore('settings').put({ key, value, version: 2, updatedAt: now() });
  ...
  const findSmallAccountBackend = async (registryBaseUrl, connectToken = '') => {
    ...
    return items.find((item) => item && String(item.customerId || item.label || item.workspace || '').includes(E2E_CNB_USERNAME) && item.url) || null;
  };

  window.__moranPreseedNovelInjectionE2E = async (options = {}) => {
    ...
    const objectStorage = {
      ...
      bucket: options.objectStorage?.bucket || 'e2e-bucket-placeholder',
      accessKey: options.objectStorage?.accessKey || 'e2e-access-key-placeholder',
      secretKey: options.objectStorage?.secretKey || 'e2e-secret-key-placeholder',
      username: options.objectStorage?.username || E2E_PLAYER_NAME,
      prefix: options.objectStorage?.prefix || 'MoRanJiangHu'
    };

    const cnbBackend = options.cnbBackend || await findSmallAccountBackend(options.registryBaseUrl || window.location.origin, options.connectToken || '').catch(() => null);
    ...
    await putSetting(db, 'game_settings', { 字数要求: 1500, 字数不足处理方式: '仅提示', 启用标签检测完整性: true, 启用标签修复: true });
    await putSetting(db, 'memory_settings', { 短期记忆阈值: 30, 中期记忆阈值: 50, 重要角色关键记忆条数N: 20, NPC记忆总结阈值: 20, 即时消息上传条数N: 10 });
  ...
  window.__moranNovelInjectionE2EConstants = { E2E_PLAYER_NAME, E2E_PARTNER_NAME, E2E_CNB_USERNAME };
})();
