...
export const WebDAVSyncPanel: React.FC = () => {
    ...
    const [progress, setProgress] = useState<WebDAV同步进度 | null>(null);
    ...
    const readProgress = (value: WebDAV同步进度) => {
        setProgress(value);
    ...
    const handleImportSave = () => runTask('import-save', async () => {
        ...
        const active = await persistConfig();
        const { save } = await 下载WebDAV云存档(active, selected, readProgress);
    ...
    const handleRestoreSettings = () => runTask('restore-settings', async () => {
        ...
        const active = await persistConfig();
        const result = await 下载设置自WebDAV(active, readProgress);
        if (result.success) {
