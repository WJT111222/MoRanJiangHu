...
import { buildSyncApiUrl, requiresRemoteSyncApi } from '../../../utils/nativeRuntime';
...
const 构建NovelAI代理测试地址 = (): string => {
    if (typeof window === 'undefined') return '/api/novelai/ai/generate-image';
    if (requiresRemoteSyncApi()) {
        const configured = buildSyncApiUrl('/api/novelai/ai/generate-image').replace(/\/+$/, '');
        if (/^https?:\/\//i.test(configured)) return configured;
        const websiteUrl = (RELEASE_INFO.websiteUrl || 'https://msjh.bacon159.pp.ua').replace(/\/+$/, '');
        return `${websiteUrl}/api/novelai/ai/generate-image`;
    }
...
const 测试NovelAI官方连接 = async (apiKey: string): Promise<string> => {
    ...
    const response = await fetch(构建NovelAI代理测试地址(), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Accept: 'application/json',
    ...
    if (response.status === 404) {
        throw new Error('当前站点没有命中 /api/novelai 代理，请确认网站已部署 NovelAI 代理函数。');
    }
    if ((response.status === 400 || response.status === 422) && !authFailure) {
        return 'NovelAI 连接成功：代理可用，Token 已通过上游鉴权。';
    }
    if (!response.ok) {
        throw new Error(`NovelAI 代理请求失败：HTTP ${response.status}${detail ? `\n${detail.slice(0, 500)}` : ''}`);
    }
    return 'NovelAI 连接成功：代理可用，Token 可用。';
};
...
const ImageGenerationSettings: React.FC<Props> = ({ settings, onSave }) => {
    ...
    const handleTestImageConnection = async () => {
        ...
        try {
            ...
            if (backend === 'novelai') {
                setMainConnectionMessage(await 测试NovelAI官方连接(apiKey));
                return;
            }
    ...
    const handleTestNsfwImageConnection = async () => {
        ...
        try {
            ...
            if (backend === 'novelai') {
                setNsfwConnectionMessage(`NSFW ${await 测试NovelAI官方连接(apiKey)}（${base}）`);
                return;
            }
            const message = await 测试OpenAI兼容图片接口({
                rawBaseUrl: base,
