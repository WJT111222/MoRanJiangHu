...
const WorkflowGraphSettings: React.FC<{
    ...
    const normalized = useMemo(() => 规范化接口设置(settings), [settings]);
    ...
    const stageCard = (stage: FlowStage) => {
        ...
        const className = `h-[150px] w-[210px] shrink-0 overflow-hidden rounded border px-3 py-2 text-left shadow-inner transition ${statusClass[stage.status]} ${
            clickable ? 'cursor-pointer hover:-translate-y-0.5 hover:border-wuxia-gold/70 focus:outline-none focus:ring-2 focus:ring-wuxia-gold/60' : ''
        }`;
        const content = (
            <>
            ...
            <div className="mt-2 space-y-0.5 text-[10px] leading-4">
                <div className="text-gray-300">渠道：<span className="text-wuxia-cyan">{stage.channel}</span></div>
                <div className="text-gray-300">模型：<span className="text-wuxia-gold">{stage.model}</span></div>
                <div className="max-h-[34px] overflow-hidden text-gray-500">{stage.note}</div>
                {clickable && <div className="pt-1 text-[10px] text-wuxia-gold">点击配置此阶段</div>}
            </div>
            </>
        );
        if (clickable) {
