...
仓库使用“本地 `.env` 文件 + Cloudflare Wrangler Secrets”的协同方式：

- 本地开发：复制 `.env.production.example` 为 `.env.local`，只填写前端构建需要的 `VITE_*` 变量。
- 本地 Worker 调试：复制 `.dev.vars.example` 为 `.dev.vars`，供 `wrangler dev` 读取运行时变量。
- 云端 Worker：把 `.env.production` 批量导入 Cloudflare Secrets，不把真实密钥提交到 Git。
- `.env`、`.env.*`、`.dev.vars*` 已被忽略；只提交 `.env.production.example` 与 `.dev.vars.example`。

常用命令：

```bash
npm run cf:secrets:bulk -- .env.production
...
VITE_GITHUB_NATIVE_CLIENT_ID=your_native_github_oauth_client_id
...
- `VITE_GITHUB_NATIVE_CLIENT_ID` 用于 APK / 原生壳 GitHub OAuth 登录。
...
| 类型 | 变量 | 用途 |
| --- | --- | --- |
| Secret | `GITHUB_CLIENT_ID` | GitHub OAuth 服务端 client id |
| Secret | `GITHUB_CLIENT_SECRET` | GitHub OAuth 服务端 client secret |
| Secret | `GITHUB_NATIVE_CLIENT_ID` | APK / 原生壳 GitHub OAuth client id |
| Secret | `GITHUB_NATIVE_CLIENT_SECRET` | APK / 原生壳 GitHub OAuth client secret |
| Secret | `FANDOM_PRESET_GITHUB_TOKEN` | 社区预设自动提交 PR 所需 GitHub Token |
| Secret | `IMAGE_HOST_TOKEN` | 图床上传与下载代理鉴权 Token |
| Var | `FANDOM_PRESET_REPO_OWNER` | 社区预设仓库 owner |
| Var | `FANDOM_PRESET_REPO_NAME` | 社区预设仓库名 |
| Var | `FANDOM_PRESET_BASE_BRANCH` | 社区预设目标分支 |

`wrangler.jsonc` 已包含 R2、静态资源和非敏感默认值，例如 `CNB_SYNC_R2`、`CNB_SYNC_R2_KEY`、`ONLINE_SESSIONS_R2_PREFIX`、`ASSETS`。不要把 OAuth secret、GitHub token、图床 token 写入 `wrangler.jsonc`。

### GitHub OAuth 部署检查

如果你使用 `.github/workflows/deploy-worker.yml` 通过 GitHub Actions 构建再部署到 Cloudflare Worker，需要同时配置下面两类变量：

- GitHub Actions Secret：`VITE_GITHUB_CLIENT_ID`、`VITE_GITHUB_NATIVE_CLIENT_ID`、`VITE_SYNC_API_BASE_URL`
- Cloudflare Worker Secrets：`GITHUB_CLIENT_ID`、`GITHUB_CLIENT_SECRET`、`GITHUB_NATIVE_CLIENT_ID`、`GITHUB_NATIVE_CLIENT_SECRET`
...
| 前端公开环境变量 | `VITE_GITHUB_NATIVE_CLIENT_ID` | APK / 原生壳 GitHub OAuth 登录 |
...
| Cloudflare 运行环境 | `GITHUB_NATIVE_CLIENT_ID` | 原生 OAuth 服务端交换 access token |
| Cloudflare 运行环境 | `GITHUB_NATIVE_CLIENT_SECRET` | 原生 OAuth 服务端交换 access token |
