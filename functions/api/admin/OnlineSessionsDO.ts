...
export class OnlineSessionsDO {
    ...
    private async handleHeartbeat(request: Request): Promise<Response> {
        ...
        const record: any = {
            ...
            ip: readStr(body?.clientIp) || getClientIp(request) || existing?.ip || '',
            country: readStr(body?.clientCountry) || readStr(cf.country) || existing?.country || '',
            region: readStr(body?.clientRegion) || readStr(cf.region) || readStr(cf.regionCode) || existing?.region || '',
            city: readStr(body?.clientCity) || readStr(cf.city) || existing?.city || '',
            timezone: readStr(body?.clientTimezone) || readStr(cf.timezone) || existing?.timezone || '',
            colo: readStr(body?.clientColo) || readStr(cf.colo) || existing?.colo || '',
            userAgent: (readStr(body?.clientUserAgent) || readStr(request.headers.get('User-Agent')) || existing?.userAgent || '').slice(0, 240),
