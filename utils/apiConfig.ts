...
const 是否旧默认无独立负面ComfyUI工作流 = (value: string): boolean => {
    ...
    return /ConditioningZeroOut/i.test(text)
        && /mPMix_NSFW_V9_fp8/i.test(text)
        && /qwen_3_4b\.safetensors/i.test(text)
        && /qwen-image-2512-Q6_K/i.test(text);
};
...
const 标准化功能模型占位 = (raw: any): 功能模型占位配置结构 => {
    ...
    return {
        ...
        ComfyUI工作流JSON: 规范化普通ComfyUI工作流JSON(raw?.ComfyUI工作流JSON),
...
export const 获取文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    const result: 当前可用接口结构 = {
        ...
        ComfyUI工作流JSON: 规范化普通ComfyUI工作流JSON(feature?.ComfyUI工作流JSON)
    };
