...
const 比较展示保存时间 = (a: 对象存储云存档元数据, b: 对象存储云存档元数据): number => {
    const happenedDiff = 读取存档发生时间(a) - 读取存档发生时间(b);
    if (happenedDiff !== 0) return happenedDiff;
    return 比较代表新旧(a, b);
};
...
export const 构建对象存储云存档时间树 = (saves: 对象存储云存档元数据[]): 对象存储时间树系列[] => {
    ...
    return Array.from(groups.entries())
        .map(([key, items]) => {
            ...
            const latest = [...nodes].sort((a, b) => 比较展示保存时间(b, a))[0]
                || [...items].sort((a, b) => 比较展示保存时间(b, a))[0];
            return {
                key,
        ...
        .sort((a, b) => 比较展示保存时间(b.latest, a.latest));
};
