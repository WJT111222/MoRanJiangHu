...
export const 获取境界层级 = (textRaw: string, cfg?: 境界配置): number => {
    ...
    if (config.stageNames && config.tierAliases && config.levelNames.length > 0) {
        ...
        if (stageMatch) {
            const stageIdx = config.stageNames.indexOf(stageMatch[1]);
            const tierVal = Number(Object.entries(config.tierAliases).find(([, v]) => v === (config.tierAliases as Record<string, string>)[stageMatch[2]])?.[0] || '一'.charCodeAt(0));
            if (stageIdx >= 0) {
                const numericTier = { '一': 1, '二': 2, '三': 3, '四': 4 }[(config.tierAliases as Record<string, string>)[stageMatch[2]] || '一'] || 1;
                const stageLevel = stageIdx * 4 + numericTier;
                if (stageLevel > 0 && stageLevel <= config.levelNames.length) return stageLevel;
            }
        }
    }
    let rank = 1;
