...
describe('imageGenerationDiagnostics', () => {
    ...
    it('keeps direct OpenAI-compatible image endpoints in local web debug when proxy is not explicitly enabled', () => {
        ...
        try {
            ...
            expect(构建OpenAI图片生成端点('https://api.openai.com/v1', undefined, { 图片需要代理: true, 供应商ID: 'openai-official' }))
                .toBe('http://127.0.0.1:4173/api/image-backend/openai-image-proxy/v1/images/generations?provider=openai-official');
        } finally {
