...
describe('B2 release publish manifest script', () => {
    it('defaults the APK manifest to the OneDrive provider while keeping B2 explicit', () => {
        const source = readFileSync(scriptPath, 'utf8');

        expect(source).not.toContain("readEnv('MORAN_RELEASE_PREFERRED_APK_PROVIDER', 'hi168')");
        expect(source).toContain("readEnv('MORAN_RELEASE_PREFERRED_APK_PROVIDER', 'onedrive')");
        expect(source).toContain("preferredApkProvider !== 'b2'");
