...
【硬约束】
1. primaryCurrency、progressionNames、defaultBackgrounds、defaultTalents、organizationName 这5个字段必须填写，缺失视为失败。
2. progressionNames 至少3个、至多9个，必须从低到高排列。
3. 不要输出与小说内容无关的通用回答；所有值必须来自或贴合小说数据集摘要中的信息。
4. 如果原著信息不足以推断某个字段的值，不要编造，直接省略该字段。
5. 只有数据集明确出现“无限流、主神空间、轮回者、任务世界、奖励点、支线剧情”等证据时，才允许输出主神/无限流相关词；否则不得照抄字段说明里的题材示例。
`.trim();
