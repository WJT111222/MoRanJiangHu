...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '10. 正文可用代称，变量层必须使用 2-4 个中文汉字真实姓名，并用 `曾用名/身份/简介/记忆` 建立代称到真名的一一对应；有独立对白框的女性、长期关系对象、关键承接对象，要按 NPC 协议同步补齐外貌、身材、衣着、称呼、关系突破条件、私密档案和名器档案。',
            ...
            '19. `社交[i].姓名` 必须是该人物的真实姓名，使用 2-4 个中文汉字；禁止把“自己/他/她/对方/那人/黑衣女子/某护卫/自己已经没有”等代称、身份、短句或正文动作写进姓名。正文里可以继续用代称，但变量里要建立“真实姓名 + 身份/简介/曾用名”的一一对应；若只知道代称，也要先为该 NPC 起一个可持续使用的真实姓名，把代称放到身份、简介、记忆或曾用名。',
