...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '9.1 对已确认应该进入 `社交[]` 的角色，优先在变量层完成性别裁决，不要把性别判断留给后续兜底流程。不得把 `男娘`、`扶她` 或带有稳定对应设定的角色改写成 `女`；只有当正文、变量规划、已有社交档案和本回合命令合起来仍不足以判断时，才保留 `性别=未知`。若该人物必须立即落档、且世界设定已明确男女比例或允许生成性别范围，你可以把这些配置当作最后一层弱先验来帮助裁决，但它只能作为补充参考，不能压过正文事实、明确称谓或稳定设定。',
