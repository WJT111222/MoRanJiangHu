...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '10. 正文可用代称，变量层必须使用 2-4 个中文汉字真实姓名；代称、身份称呼、外貌描述应优先写入 `身份/简介/记忆`，只有确有旧称、化名、曾用称呼时才写 `曾用名`，不要给每个 NPC 强行生成曾用名。新建女性 NPC 时必须从本回合注入的【女性新角色姓名候选池】中选择，不要自造“婉儿/清雪/若嫣/灵儿/月儿”等模板名；有独立对白框的女性、长期关系对象、关键承接对象，要按 NPC 协议同步补齐外貌、身材、衣着、称呼、关系突破条件、私密档案和名器档案。',
            '11. 每回合都要复核主角与 NPC 档案：主角必须具备 `角色.技艺`，所有 NPC 必须具备天赋列表、出身背景、当前装备、背包、BUFF、DEBUFF、技艺、七部位血量与状态；技艺至少覆盖炼器、炼丹、医术、阵法、符箓、机关、采集、鉴定等常见项。',
            '12. 若本次开局配置或当前存档题材模式为仙侠，主角与 NPC 还必须维护 `灵根/灵根资质/当前灵力/最大灵力/当前神识/最大神识/丹田状态/道基状态/心魔值/功德/业力`；术法、神通、法宝、阵法、符箓、神识探查等事件必须同步消耗或恢复灵力/神识。',
            '13. 仙侠模式下，底层变量仍写 `角色.金钱.铜钱/银子/金元宝`，但语义分别是下品灵石/中品灵石/上品灵石；正文和变量规划里的价格、奖励、拍卖、消耗应使用灵石口径，不再写元宝、银子、铜钱。',
            ...
            '19. `社交[i].姓名` 必须是该人物的真实姓名，使用 2-4 个中文汉字；女性新角色必须从本回合注入的【女性新角色姓名候选池】中选择，同一存档内不得重复。已经存在的姓名、玩家手动改过的姓名、正文已稳定承接的姓名必须原样保留，不得为了候选池、风格统一或“更合理”而改名；即使老存档里已有“苏婉儿/婉儿/清雪”等模板名，也不要主动重命名。只有当前姓名是占位/代称，且本回合正文明确揭示真名时，才允许 `set 社交[i].姓名`。禁止把“自己/他/她/对方/那人/黑衣女子/某护卫/自己已经没有”等代称、身份、短句或正文动作写进姓名。正文里可以继续用代称，但变量里要建立“真实姓名 + 身份/简介/记忆”的稳定对应；若只知道代称，也要先为该 NPC 起一个可持续使用的真实姓名，把代称放到身份、简介或记忆。`曾用名` 只用于真实存在的旧称、化名、曾用称呼，不得为了凑字段给每个 NPC 都写。',
            '20. 若正文明确确认某个 NPC 死亡、身亡、阵亡、战死、气绝、断气、毙命或已故，必须同步写回该 NPC：`当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并在 `DEBUFF` 中保留“死亡”状态；“昏死、濒死、重伤、险些身亡、差点死、未死”不得按死亡处理。',
            ...(survivalNeedsEnabled
                ? [[
                    '21. 生理系统开启时，要把时间推进、休整、进食、饮水、赶路、熬战等事实对应到精力',
                    构建修炼体系附加块('、内力'),
                    '、饱腹、口渴等变量联动。'
                ].join('')]
                : [])
