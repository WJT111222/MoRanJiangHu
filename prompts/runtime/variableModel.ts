...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '21. 死亡/已故判定必须反复核对，只有同时满足三项才允许写入：目标 NPC 身份/姓名/索引一致；该 NPC `当前血量 = 0`；正文明确写出此人已经死亡、身亡、阵亡、战死、气绝、断气、毙命或已故；并且正文或变量命令给出明确死因（如被谁击杀、中毒、感染、失血、伤势过重等）。满足条件时必须同步写回 `当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并把明确死因写入 `死因/死亡原因/死亡描述/记忆/DEBUFF` 至少一处；缺少明确死因、只是昏死/濒死/重伤/险些身亡/差点死/未死/失踪/状态未知时，一律不得写“死亡/已故”。',
