...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '11. 每回合都必须刷新当前镜头快照：先确认 `环境` 的当前描写视角/当前位置，再根据本回合正文与对白只把明确在当前现场、出声、行动、被镜头点到、站在主角身边的人设为 `社交[i].是否在场 = true`；其余旧在场 NPC 若本回合没有被当前现场确认，必须自然设为 `false`。远端、留守、待命、传闻、背景名单、曾经出现过的人都不是在场。',
            '12. 若正文明确确认某个 NPC 死亡、身亡、阵亡、战死、气绝、断气、毙命或已故，必须同步写回该 NPC：`当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并在 `DEBUFF` 中保留“死亡”状态；“昏死、濒死、重伤、险些身亡、差点死、未死”不得按死亡处理。',
