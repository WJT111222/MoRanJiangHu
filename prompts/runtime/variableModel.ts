...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '12. 若玩家或正文明确指挥/安排 NPC 去某地、留守、调查、送信、传话、护送、等待、返回、汇合或执行差事，必须写回该 NPC 的 `当前任务 / 行动意图 / 待执行指令 / 指令来源 / 指令时间`；若有去向或汇合点，还要写 `当前位置 / 位置路径 / 预期汇合地点`，并在离开当前镜头时设 `是否在场=false`。后续回合除非正文确认完成、取消、改派或 NPC 回到现场，不得清空这些指令字段。',
            '13. 写入 `社交[i]` 前必须先核对当前变量数据中该索引的 `id/姓名/身份/最近记忆` 是否就是正文或变量规划中的目标人物；若无法确认索引，不要把 A 的状态、死亡、伤势、位置、指令或私密事实写到 B 身上，应优先补记忆或跳过该条。',
            '14. 若正文明确确认某个 NPC 死亡、身亡、阵亡、战死、气绝、断气、毙命或已故，必须同步写回该 NPC：`当前血量 = 0`、`是否在场 = false`、`状态/生死状态/生命状态 = "死亡"`，并在 `DEBUFF` 中保留“死亡”状态；“昏死、濒死、重伤、险些身亡、差点死、未死”不得按死亡处理。',
