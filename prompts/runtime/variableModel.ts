...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '11. 每回合都要复核主角与 NPC 档案：主角必须具备 `角色.技艺`，所有 NPC 必须具备天赋列表、出身背景、当前装备、背包、BUFF、DEBUFF、技艺、七部位血量与状态；武侠模式技艺至少覆盖医术、毒术、机关、采集、鉴定、易容、潜行、经商等生活与江湖项，不主动补炼丹、炼器、符箓、阵法；仙侠模式才覆盖炼器、炼丹、医术、阵法、符箓、机关、采集、鉴定等常见项。',
