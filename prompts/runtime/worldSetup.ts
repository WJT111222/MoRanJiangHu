...
${构建修炼体系附加块(`- 本存档世界观必须兼容“累计境界值”的${是否仙侠开局模式(openingConfig) ? '仙侠修真' : 题材配置.label}成长主轴，并保持高境界强者的合理稀缺度与风险代价。`)}
...
2. 成长体系一致性
   - 本存档世界必须兼容当前项目的${解析生效题材配置(openingConfig?.题材模式, openingConfig?.modeRuntimeProfile).label}成长体系。
   - 高手应稀缺，高阶传承应稀缺，高收益应对应高风险、高代价或高势力门槛。
   - 设定与现有数值哲学保持一致，例如低风险地区不泛滥神功神装、宗师密度受控、越境碾压伴随代价。
${构建修炼体系附加块([
`   - 本存档世界必须兼容当前项目“累计境界值”${是否仙侠开局模式(openingConfig) ? '仙侠修真' : 解析生效题材配置(openingConfig?.题材模式, openingConfig?.modeRuntimeProfile).label}成长体系。`,
...
6. 势力格局
   ...
   - 每个势力有代表性的物品风格描述；物品池必须贴合${解析生效题材配置(openingConfig?.题材模式, openingConfig?.modeRuntimeProfile).label}，参考：${解析生效题材配置(openingConfig?.题材模式, openingConfig?.modeRuntimeProfile).presetItemKeywords.join('、')}。
   - 势力之间的关系网是世界演化和${解析生效题材配置(openingConfig?.题材模式, openingConfig?.modeRuntimeProfile).auctionName}物品流通的基础。
