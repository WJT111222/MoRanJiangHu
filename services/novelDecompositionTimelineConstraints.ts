...
const 跨世界关键词正则 = /(无限流|主神|轮回|副本|任务世界|剧情世界|异世界|平行世界|不同世界|跨世界|进入.+世界|回归|穿越|时间流速|流速|相隔|间隔|世界线)/u;
...
export const 小说拆分疑似跨世界题材 = (dataset: 小说拆分数据集结构): boolean => (
    收集候选文本(dataset).some((line) => 跨世界关键词正则.test(line))
);

export const 构建小说拆分跨世界时间线规则 = (dataset: 小说拆分数据集结构): string[] => {
    const candidates = 收集候选文本(dataset);
    const evidence = candidates
        .filter((line) => 跨世界关键词正则.test(line) && 时间关键词正则.test(line))
        .slice(0, 12);
    const hasCrossWorld = evidence.length > 0 || 小说拆分疑似跨世界题材(dataset);
    if (!hasCrossWorld) return [];

    return 去重文本列表([
        '跨世界时间线硬约束：章节距离不等于世界内时间距离；不得因为两段原文相邻，就把不同世界、不同副本或同一任务世界两次进入之间的原著间隔压缩。',
        '必须分开记录主世界/主神空间时间、任务世界本地时间、进入/离开世界事件与两次进入同一世界之间的本地间隔；原文写明“相隔200年”时必须按200年处理，不得改写成一个月或数日。',
