...
const OBJECT_STORAGE_INLINE_PACKAGE_LIMIT = 700 * 1024;
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
        const response = await fetch(构建对象存储代理地址(), {
            ...
            headers: {
                ...
                'X-Object-Storage-Key': 构建对象键(config, segments),
...
const 写入包 = async (
    ...
    if (inlineBody.length <= OBJECT_STORAGE_INLINE_PACKAGE_LIMIT) {
...
export const 增量同步到对象存储 = async (config: 对象存储同步配置, saves?: 存档结构[], onProgress?: 对象存储同步进度回调): Promise<对象存储增量同步结果> => {
    ...
    for (const save of localSaves) {
        ...
        await 写入包(normalized, [OBJECT_STORAGE_SAVES_DIR, metadata.fileName], packageBase, 字节转Base64(archiveBytes), 'saves', onProgress);
