...
const 写入包 = async (
    ...
    if (archiveBase64.length <= OBJECT_STORAGE_FALLBACK_CHUNK_THRESHOLD) {
        ...
        onProgress?.({ stage: 'upload', message: '数据包较大，正在使用分片上传' });
    }
...
const 写入字节包 = async (
    ...
    const inlineBase64Limit = Math.floor(OBJECT_STORAGE_FALLBACK_CHUNK_THRESHOLD * 3 / 4);
    if (archiveBytes.length <= inlineBase64Limit) {
        await 写入包(config, targetSegments, payload, 字节转Base64(archiveBytes), chunkScope, onProgress);
        return;
    }
    ...
    const rawChunkSize = Math.floor(OBJECT_STORAGE_CHUNK_BASE64_SIZE * 3 / 4);
    const total = Math.ceil(archiveBytes.length / rawChunkSize);
    for (let index = 0; index < total; index += 1) {
        const partBytes = archiveBytes.subarray(index * rawChunkSize, (index + 1) * rawChunkSize);
        const part = 字节转Base64(partBytes);
        const fileName = `part-${String(index + 1).padStart(4, '0')}.txt`;
        onProgress?.({
            stage: 'upload',
            current: index + 1,
...
const 打包对象存储存档 = async (
    ...
    const archiveBlob = await 导出ZIP存档文件({
        saves: bundledSaves,
...
export const 上传设置到对象存储 = async (config: 对象存储同步配置, onProgress?: 对象存储同步进度回调): Promise<对象存储设置同步元数据> => {
    ...
    await 写入字节包(normalized, [metadata.fileName], packagePayload, bytes, 'settings', onProgress);
...
export const 增量同步到对象存储 = async (
    config: 对象存储同步配置,
    saves?: 存档结构[],
    onProgress?: 对象存储同步进度回调,
    options?: { includeImages?: boolean; includeAncestors?: boolean }
): Promise<对象存储增量同步结果> => {
    const normalized = 规范化配置(config);
    if (!normalized) throw new Error('请填写对象存储端点、存储桶、Access Key 和 Secret Key');
    ...
    const shouldIncludeAncestors = options?.includeAncestors !== false;
    const allLocalSaves = Array.isArray(saves) && shouldIncludeAncestors
        ? await dbService.读取存档列表().catch(() => requestedSaves)
        : requestedSaves;
    const localSaves = Array.isArray(saves) && shouldIncludeAncestors
    ...
    const packagedSaves = await 并发映射(savesNeedingPackaging, OBJECT_STORAGE_UPLOAD_CONCURRENCY, async (save, index) => {
        ...
        return 打包对象存储存档(save, localSaves, options);
    });
