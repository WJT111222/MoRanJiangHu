...
const 题材中性行情模板: Array<Omit<拍卖行情, 'ID' | '过期时间'>> = [
    { 标题: '大宗到货', 描述: '近日市面上集中到了一批常用货品，基础物件价格小幅回落。', 影响类型: '全部', 价格倍率: 0.94, 热点标签: '大宗到货' },
    { 标题: '行市走俏', 描述: '买家集中出手，常用消耗品补货不及，价格顺势抬头。', 影响类型: '消耗品', 价格倍率: 1.18, 热点标签: '行市走俏' },
    { 标题: '用料紧俏', 描述: '多家作坊同时补料，材料类货品挂出便有人问价。', 影响类型: '材料', 价格倍率: 1.2, 热点标签: '用料紧俏' },
    { 标题: '行市清淡', 描述: '大买家收手观望，饰物与雅玩更容易压价成交。', 影响类型: '饰品', 价格倍率: 0.92, 热点标签: '行市清淡' }
];

export const 获取题材行情模板 = (mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): Array<Omit<拍卖行情, 'ID' | '过期时间'>> => {
    if (runtimeProfile && 是否自定义模式运行时配置(runtimeProfile, mode)) {
        const custom = runtimeProfile.economy?.marketEventTemplates;
        if (Array.isArray(custom) && custom.length > 0) {
            return custom.map((item) => ({
                标题: item.标题,
                描述: item.描述,
                影响类型: (item.影响类型 || '全部') as 拍卖行情['影响类型'],
                价格倍率: item.价格倍率,
                热点标签: item.热点标签 || item.标题
            }));
        }
        return 题材中性行情模板;
    }
...
const 是否不符合题材拍卖物品 = (item: any, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): boolean => {
    if (runtimeProfile && 是否自定义模式运行时配置(runtimeProfile, mode)) {
        const banned = runtimeProfile.economy?.bannedKeywords || [];
        if (banned.length === 0) return false;
        const text = [item?.名称, item?.描述, item?.类型, item?.品质, ...(Array.isArray(item?.标签) ? item.标签 : []), ...(Array.isArray(item?.市场标签) ? item.市场标签 : [])].filter(Boolean).join(' ');
        return banned.some((keyword) => keyword && text.includes(keyword));
    }
    const profile = 获取题材模式配置(mode);
...
const 是否不符合题材拍卖记录 = (entry: any, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): boolean => 是否不符合题材拍卖物品({
    ...(entry?.物品 || {}),
    市场标签: entry?.市场标签
}, mode, runtimeProfile);
...
const 规范化拍卖行状态 = (parsed: Partial<拍卖行状态> | null | undefined, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): 拍卖行状态 => 清理并补货({
...
}, { 允许系统补货: false, 题材模式: mode, 模式运行时: runtimeProfile }); // 读取时不自动补货，等待剧情触发
...
export const 读取拍卖行状态 = (scope?: string, options?: { 题材模式?: 拍卖题材模式; 模式运行时?: ModeRuntimeProfile | null }): 拍卖行状态 => {
    if (typeof window === 'undefined') return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式, 模式运行时: options?.模式运行时 });
    try {
        const scopedKey = 获取拍卖行存储键(scope);
        const raw = window.localStorage.getItem(scopedKey);
        if (!raw) return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式, 模式运行时: options?.模式运行时 });
        const parsed = JSON.parse(raw) as Partial<拍卖行状态>;
        return 规范化拍卖行状态(parsed, options?.题材模式, options?.模式运行时);
    } catch {
        return 清理并补货(创建空拍卖行状态(), { 允许系统补货: false, 题材模式: options?.题材模式, 模式运行时: options?.模式运行时 });
    }
};
...
export const 创建默认拍卖行状态 = (mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): 拍卖行状态 => 清理并补货(创建空拍卖行状态(), { 允许系统补货: true, 题材模式: mode, 模式运行时: runtimeProfile });

const 行情是否不符合题材 = (market: { 标题?: string; 描述?: string; 热点标签?: string }, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): boolean => {
...
const 清理题材行情列表 = (previous: 拍卖行情[] = [], mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): 拍卖行情[] => {
    const now = Date.now();
    const templateKeys = new Set(获取题材行情模板(mode, runtimeProfile).map((item) => `${item.标题}|${item.热点标签}`));
    return previous.filter((item) => {
        if (!item || item.过期时间 <= now) return false;
        if (行情是否不符合题材(item, mode, runtimeProfile)) return false;
        return templateKeys.has(`${item.标题}|${item.热点标签}`);
    });
};

export const 生成行情列表 = (force = false, previous: 拍卖行情[] = [], previousTime = 0, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): { 行情列表: 拍卖行情[]; 最近行情时间: number } => {
    const now = Date.now();
    const activePrevious = 清理题材行情列表(previous, mode, runtimeProfile);
    if (!force && activePrevious.length > 0 && now - previousTime < 6 * HOUR_MS) {
        return { 行情列表: activePrevious, 最近行情时间: previousTime || now };
    }
    const eligibleTemplates = 获取题材行情模板(mode, runtimeProfile)
        .filter((template) => !行情是否不符合题材(template, mode, runtimeProfile));
...
const 获取默认拍卖物品池 = (mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): 默认拍卖物品模板[] => {
    if (runtimeProfile && 是否自定义模式运行时配置(runtimeProfile, mode)) {
        const pool = (runtimeProfile.items?.rewardItemPool || []).map((name) => (name || '').trim()).filter(Boolean).slice(0, 6);
        if (pool.length > 0) {
            return pool.map((name, index) => ({
                名称: name,
                类型: '杂物' as const,
                品质: (index % 3 === 2 ? '上品' : index % 3 === 1 ? '良品' : '凡品') as 默认拍卖物品模板['品质'],
                描述: `${runtimeProfile.economy?.marketName || '市场'}常见的流通货品。`,
                价值: 120 + index * 90,
                标签: ['市场常货']
            }));
        }
...
const 创建默认拍卖品记录 = (template: 默认拍卖物品模板, index: number, mode?: 拍卖题材模式, runtimeProfile?: ModeRuntimeProfile | null): 拍卖品记录 => {
    const profile = 解析生效题材配置(mode, runtimeProfile);
    const now = Date.now();
    const price = Math.max(1, Math.floor(template.价值 * (0.92 + index * 0.05)));
    const 自定义卖家池 = runtimeProfile && 是否自定义模式运行时配置(runtimeProfile, mode)
        ? [(runtimeProfile.economy?.marketName || '').trim(), ...(runtimeProfile.organization?.organizationAliases || []).map((alias) => (alias || '').trim())].filter(Boolean)
        : [];
    const sellerName = 自定义卖家池.length > 0
        ? 自定义卖家池[index % 自定义卖家池.length]
...
export const 清理并补货 = (state: 拍卖行状态, options?: { 允许系统补货?: boolean; 最大系统补货数量?: number; 目标在售数量?: number; 题材模式?: 拍卖题材模式; 模式运行时?: ModeRuntimeProfile | null }): 拍卖行状态 => {
    const now = Date.now();
    const 行情 = 生成行情列表(false, state.行情列表 || [], state.最近行情时间, options?.题材模式, options?.模式运行时);
    const cleaned = (state.拍卖品列表 || [])
        ...
        .filter((entry) => {
            ...
            return !autoMarketEntry || !是否不符合题材拍卖记录(entry, options?.题材模式, options?.模式运行时);
        })
        .map((entry) => (
...
const 标准化事件物品 = (raw: Partial<游戏物品>, fallbackName: string, fallbackPrice: number, fallbackDescription?: string): 游戏物品 => {
    ...
    return {
        ...
        描述: 读文本(raw?.描述, fallbackDescription || '由江湖事件流入市面的稀罕物。'),
        类型: type,
        品质: quality,
...
export const 创建事件拍卖品 = (params: 拍卖行事件投放参数): 拍卖品记录 => {
    ...
    const custom = Boolean(params.模式运行时 && 是否自定义模式运行时配置(params.模式运行时, params.题材模式));
    const item = 标准化事件物品({
        ...picked,
        价值: price,
    }, `${params.事件名称 || (custom ? '市面事件' : '江湖事件')}遗物`, price, custom ? '由近期事件流入市面的稀罕物。' : undefined);
    ...
    return {
        ...
        卖家名称: 读文本(params.卖家名称, custom ? 读文本(params.模式运行时?.economy?.marketName, '市面掮客') : '江湖掮客'),
...
export const 投放事件拍卖品 = (state: 拍卖行状态, params: 拍卖行事件投放参数): 拍卖行状态 => {
    const cleaned = 清理并补货(state, { 允许系统补货: false, 题材模式: params.题材模式, 模式运行时: params.模式运行时 });
    ...
    if (是否不符合题材拍卖物品(auction.物品, params.题材模式, params.模式运行时)) {
        recordDiagnosticLog('info', ['拍卖行事件投放已拦截', auction.物品?.名称 || '', auction.物品?.类型 || '', `${解析生效题材配置(params.题材模式, params.模式运行时).label}题材不投放冲突物品`]);
        return cleaned;
    }
    ...
    return {
        ...
        交易记录: [
            创建交易记录('事件投放', '事件货品入市', `「${auction.物品?.名称 || '无名物品'}」因「${params.事件名称}」${解析生效题材配置(params.题材模式, params.模式运行时).marketVerb}。`),
            ...(cleaned.交易记录 || []),
        ].slice(0, 40),
    };
};
...
export const 投放事件拍卖品并保存 = (params: 拍卖行事件投放参数, scope?: string): 拍卖行状态 => {
    const next = 投放事件拍卖品(读取拍卖行状态(scope, { 题材模式: params.题材模式, 模式运行时: params.模式运行时 }), params);
    保存拍卖行状态(next, scope);
...
export const 从势力互动投放拍卖品 = (
    ...
    options?: { scope?: string; 题材模式?: 拍卖题材模式; 模式运行时?: ModeRuntimeProfile | null }
): 拍卖行状态 => {
    ...
    for (const item of pendingItems) {
        ...
        nextState = 投放事件拍卖品(nextState, {
            ...
            题材模式: options?.题材模式,
            模式运行时: options?.模式运行时,
            卖家名称: item.来源势力 || (options?.模式运行时 && 是否自定义模式运行时配置(options.模式运行时, options?.题材模式)
                ? `${(options.模式运行时.economy?.marketName || '市场').trim()}散货`
                : '江湖散货'),
