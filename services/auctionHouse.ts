...
const 猜测主线类型 = (text: string): 主线流向 | undefined => {
    ...
    if (/宗门|门派|师门|藏经|传承|掌门|外宗|别派|剑宗|刀宗|药宗|禅宗/u.test(text)) return '宗门线';
...
const 世界流通来源语义正则 = /世界大事|江湖风闻|天下|局势|宗门|门派|外宗|别派|帮会|商会|镖局|商队|行商|坊市|黑市|牙行|拍卖行|官府|军械坊|铸坊|药铺|矿场|矿道|战后|攻杀|争夺|劫镖|互市|交易|缴获|抄没|战利品|库藏|押运|出货|流入/u;
const 持有描述物品名正则 = /你|自己|自身|身边|枕边|怀中|手中|腰间|腰侧|背包|包袱|行囊|佩着|握着|拿着|提着|端着|背着|抱着|拎着|托着|穿着|装备|放在|收在|藏在/u;
const 禁止当作物品名正则 = /旁白|西厢房|东厢房|卧房|厢房|宗门线|官府线|秘境线|江湖线|剧情|线索|名门之后|清晨|寒意|窗纸|木窗|棉被|床头|霜色|一股|股与|年龄不符|浓浓的|淡淡的|微微的|隐隐的|阵阵|缕缕|丝丝|手中|提着|拿着|端着|背着|抱着|拎着|托着/u;
...
const 是否有效物品名 = (name: string): boolean => {
    ...
    if (持有描述物品名正则.test(name)) return false;
...
const 是否合理拍卖来源上下文 = (text: string, itemName: string): boolean => {
    if (!是否具体物品入市(text, itemName)) return false;
    const context = 读取物品附近文本(text, itemName, 86);
    const explicitPlayerConsignment = 玩家私有物品语义正则.test(context)
        && /(寄售|上架|挂售|卖给|出售给|交给牙行|送入拍卖行)/u.test(context);
    if (explicitPlayerConsignment) return true;
    if (玩家私有物品语义正则.test(context)) return false;
    return 世界流通来源语义正则.test(context);
};
...
export const 从剧情响应构建拍卖行投放参数列表 = (
    ...
    if (context?.useAIExtraction && context?.aiExtractionResult) {
        ...
        const aiItems = Array.isArray(aiResult.提取的物品列表)
            ? aiResult.提取的物品列表
                .map((item: any) => {
                    const 名称 = 清理物品名候选(String(item?.名称 || ''));
                    const 类型 = 限制类型(item?.类型, 猜测物品类型(名称));
                    return { ...item, 名称, 类型 };
                })
                .filter((item: any) => item.是否合理
                    && 是否有效物品名(item.名称)
                    && 是否允许自动拍卖行入市类型(item.类型)
                    && (context.allowInitialPlotSeed || 是否合理拍卖来源上下文(responseText, item.名称)))
            : [];
    ...
    const itemNames = 提取明确物品名列表(text, context?.maxCount || (isInitialPlotSeed ? 3 : 1))
        .filter((itemName) => 是否允许自动拍卖行入市类型(猜测物品类型(itemName)))
        .filter((itemName) => isInitialPlotSeed || 是否合理拍卖来源上下文(text, itemName));
