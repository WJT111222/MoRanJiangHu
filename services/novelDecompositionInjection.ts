...
const 构建分段注入文本行 = (segment: 小说拆分数据集结构['分段列表'][number], target: 小说拆分注入目标类型): string[] => {
    ...
    if (target === 'planning') {
        return [
            ...
            Array.isArray(segment.可提前铺垫) && segment.可提前铺垫.length > 0
                ...
                : '',
            Array.isArray(segment.世界观规则) && segment.世界观规则.length > 0 ? `世界观规则：${segment.世界观规则.join('；')}` : '',
            Array.isArray(segment.世界边界规则) && segment.世界边界规则.length > 0 ? `世界边界规则：${segment.世界边界规则.join('；')}` : '',
            Array.isArray(segment.人物关系) && segment.人物关系.length > 0 ? `人物关系：${segment.人物关系.join('；')}` : '',
            Array.isArray(segment.势力关系) && segment.势力关系.length > 0 ? `势力关系：${segment.势力关系.join('；')}` : '',
            Array.isArray(segment.伏笔线索) && segment.伏笔线索.length > 0 ? `伏笔线索：${segment.伏笔线索.join('；')}` : '',
            Array.isArray(segment.回收点) && segment.回收点.length > 0 ? `回收点：${segment.回收点.join('；')}` : '',
            Array.isArray(segment.章节节奏) && segment.章节节奏.length > 0 ? `章节节奏：${segment.章节节奏.join('；')}` : '',
    ...
    return [
        ...
        Array.isArray(segment.可提前铺垫) && segment.可提前铺垫.length > 0
            ? `可提前铺垫：${segment.可提前铺垫.map(格式化可见信息条目摘要).join('；')}`
            : '',
        Array.isArray(segment.物品档案) && segment.物品档案.length > 0 ? `物品档案：${segment.物品档案.map((item) => [item.名称, item.类型].filter(Boolean).join('｜')).filter(Boolean).join('；')}` : '',
        Array.isArray(segment.世界观规则) && segment.世界观规则.length > 0 ? `世界观规则：${segment.世界观规则.join('；')}` : '',
        Array.isArray(segment.世界边界规则) && segment.世界边界规则.length > 0 ? `世界边界规则：${segment.世界边界规则.join('；')}` : '',
        Array.isArray(segment.人物关系) && segment.人物关系.length > 0 ? `人物关系：${segment.人物关系.join('；')}` : '',
        Array.isArray(segment.势力关系) && segment.势力关系.length > 0 ? `势力关系：${segment.势力关系.join('；')}` : '',
