...
const 标准化背景货币列表 = (rawItems: unknown): 背景开局货币条目[] | undefined => {
    if (!Array.isArray(rawItems)) return undefined;
    const items = rawItems
        .map((item: any) => {
            if (!item || typeof item !== 'object') {
        ...
        .filter(Boolean) as 背景开局货币条目[];
    return items.length > 0 ? items : undefined;
};

const 标准化出身背景 = (raw: any, fallback = 默认背景模板): { 名称: string; 描述: string; 效果: string; 初始物品?: { 名称: string; 数量?: number; 描述?: string; 类型?: string; }[]; 可选初始物品?: { 名称: string; 数量?: number; 描述?: string; 类型?: string; }[]; 开局货币?: 背景开局货币条目[] } => {
...
const 计算背景开局货币数量 = (entry: 背景开局货币条目, seed: string): number => {
    if (typeof entry.数量 === 'number' && entry.数量 > 0) return Math.floor(entry.数量);
    const min = 规范化货币数值(entry.最小数量);
    const max = 规范化货币数值(entry.最大数量);
    if (min > 0 || max > 0) return 稳定区间整数(seed, min || max, max || min);
    return 1;
};

const 构建背景开局货币物品 = (entry: 背景开局货币条目, params: { seed: string }): any[] => {
...
const 规范化角色物品容器映射 = (rawRole?: any, options?: 玩家BUFF规范化选项): 角色数据结构 => {
    ...
    if (背景接管开局资产 && !是否已补过背景物) {
        ...
        const 背景货币物品 = (Array.isArray(背景开局货币) ? 背景开局货币 : [])
            .map((entry: 背景开局货币条目) => 构建背景开局货币物品(entry, {
