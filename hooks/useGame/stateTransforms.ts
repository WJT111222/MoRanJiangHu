...
const 背景货币物品映射 = (itemName: string, currentMoney: Record<string, number>): Record<string, number> | null => {
    const name = 规范化文本(itemName);
    const moneyKeys = Object.keys(currentMoney);
    if (!name || moneyKeys.length <= 0) return null;
    const 取首个可用货币键 = (...candidates: string[]) => candidates.find((key) => moneyKeys.includes(key));
    if (name === '盘缠') {
        const key = 取首个可用货币键('银子', '现金', '金币', '灵石', '通用点数', '中层货币', '底层货币');
        return key ? { [key]: 10 } : null;
    }
    if (name === '零钱盒') {
        const key = 取首个可用货币键('现金', '银子', '金币', '铜钱', '底层货币');
        return key ? { [key]: 50 } : null;
    }
    return null;
};
const 累加角色货币 = (role: any, additions: Record<string, number>) => {
    const current = role?.金钱 && typeof role.金钱 === 'object' ? role.金钱 : {};
    const next = { ...current };
    Object.entries(additions).forEach(([key, value]) => {
        next[key] = 规范化货币数值(next[key]) + 规范化货币数值(value);
    });
    role.金钱 = next;
};
    ...
    if (Array.isArray(背景初始物品) && !是否已补过背景物) {
        const 背景物品 = 背景初始物品
            .map((entry: any) => {
                ...
                if (!name) return null;
                const 货币映射 = 背景货币物品映射(name, (role as any).金钱 || {});
                if (货币映射) {
                    累加角色货币(role, 货币映射);
                    return null;
                }
