...
import { 构建COT伪装提示词, 酒馆预设模式可用 } from './promptRuntime';
...
export const 构建上下文快照数据 = async (params: 构建上下文快照参数): Promise<上下文快照> => {
    ...
    const builtContext = await params.构建系统提示词(
        ...
        {
            ...(recallContextMode ? { 禁用中期长期记忆: true, 禁用短期记忆: true } : {}),
            世界书作用域: 构建主剧情世界书作用域(酒馆预设模式可用(规范化游戏设置(params.gameConfig))),
            世界书附加文本: [formatHistoryToScript(snapshotScriptHistory)]
        }
    );
