...
const 创建默认同门名录 = (sectName: string, openingConfig?: OpeningConfig): 详细门派结构['重要成员'] => {
    ...
    const xianxia = openingConfig?.题材模式 === '仙侠';
    return [
        { id: 'opening_senior_sister', 姓名: '苏清寒', 职位: xianxia ? '内门师姐' : '内门弟子', 境界: xianxia ? '炼气三层' : '聚息二层', 简介: `${sectName}年轻一辈中处事稳妥的师姐，负责照看新入门弟子。`, 好感度: 35, 是否在场: true, 是否主要角色: true },
        { id: 'opening_senior_brother', 姓名: '林砚舟', 职位: xianxia ? '外门师兄' : '外门弟子', 境界: xianxia ? '炼气二层' : '开脉四重', 简介: `常在演武场和任务堂之间奔走，消息灵通。`, 好感度: 20, 是否在场: false, 是否主要角色: false },
        { id: 'opening_peer_disciple', 姓名: '许明澈', 职位: xianxia ? '新晋外门弟子' : '杂役弟子', 境界: xianxia ? '炼气一层' : '开脉二重', 简介: `与主角同期入门，性子直爽，容易被门中琐事牵着走。`, 好感度: 15, 是否在场: true, 是否主要角色: false }
    ] as any;
};
