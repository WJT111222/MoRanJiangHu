...
const 从门派任务创建通用任务列表 = (sectName: string, missions: 详细门派结构['任务列表'], openingConfig?: OpeningConfig): 任务结构[] => {
    ...
    Array.isArray(missions) ? missions.map((mission) => ({
        ...
        剧情暗线: `${isInfinite ? '团队任务：发布者一般是队长、资深者或团队协调人；奖励应是团队贡献、奖励点登记、补给额度、团队能力库权限或具体补给；契机必须来自小队协作、补给清点、侦查分工、防守加固或情报获取，不得复述主神发布的存活倒计时主线。' : isTopicOrganization ? '组织事务' : '门派任务'}：${sectName}的「${取文本(mission?.标题, isInfinite ? '小队协同任务' : isTopicOrganization ? '组织事务' : '门派差遣')}」必须结合当前剧情、地点、在场人物与${isInfinite ? '主神任务进度和小队状态' : isTopicOrganization ? '组织近况' : '门派近况'}推进。`
    })) : []
);
};
...
const 题材兑换列表是否违和 = (items: any[], organizationKind: 组织题材): boolean => {
    ...
    if (organizationKind === '营地' || organizationKind === '组织') {
        return /辟谷丹|回气丹|凝元丹|破境丹|筑基丹|引气丹|聚灵丹|混元丹|丹药|玉骨扇|丹炉|法器|灵石/u.test(text);
    }
    if (organizationKind === '轮回小队') {
        return /辟谷丹|回气丹|凝元丹|破境丹|筑基丹|引气丹|聚灵丹|混元丹|清心丹|洗髓丹|疗伤丹|护脉丹|养气丹|凝神丹|丹药|丹炉|杂役弟子|外门弟子|内门弟子|真传弟子|门派贡献|宗门贡献/u.test(text);
    }
    return false;
};
