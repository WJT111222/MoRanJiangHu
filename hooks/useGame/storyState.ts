...
const 创建默认门派任务列表 = (sectName: string): 详细门派结构['任务列表'] => ([
    {
        ...
        标题: `${sectName}临场差遣`,
        描述: '由AI结合当前剧情、地点、在场人物与门派近况生成具体目标；不得长期固定为巡山或采买。',
        类型: '日常',
        难度: '1星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:03:23:59',
    ...
    },
    {
        ...
        标题: '剧情牵引委托',
        描述: '围绕主角当前遭遇、门派人际或附近风波生成一项可承接任务。',
        类型: '建设',
        难度: '1星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:05:23:59',
    ...
    },
    {
        ...
        标题: '门中历练',
        描述: '由AI按门派等级、资源压力、敌友关系与当前剧情随机生成历练内容。',
        类型: '历练',
        难度: '2星',
        发布日期: '1:01:01:00:00',
        截止日期: '1:01:10:23:59',
...
const 创建默认同门名录 = (_sectName: string, openingConfig?: OpeningConfig): 详细门派结构['重要成员'] => {
...
const 推导门派规模数据 = (source: any, displayName: string) => {
    ...
    const level = 取文本(source?.门派等级)
        || (score >= 120 ? '一流大派' : score >= 70 ? '二流门派' : score >= 35 ? '三流门派' : score >= 12 ? '地方小派' : '草创门派');
    const scale = 取文本(source?.门派规模)
        || (total >= 500 ? '大宗门' : total >= 120 ? '中型门派' : total >= 30 ? '小型门派' : total > 0 ? '草创小门' : '待AI生成规模');
    const wealth = 取文本(source?.财富评级)
        || (funds >= 30000 ? '富甲一方' : funds >= 10000 ? '殷实' : funds >= 2500 ? '尚可' : funds > 0 ? '拮据' : '待AI评估');
...
export const 创建开局门派状态 = (
    ...
    const normalized = 规范化门派状态({
        ...
        玩家贡献: contribution,
        累计贡献: contribution,
        简介: '开局模板：请由AI按世界观、主角出身和随机性生成本局门派，不得沿用固定门派名。',
        弟子总数: 0,
        门派等级: '待AI评定',
        门派规模: '待AI生成规模',
        财富评级: '待AI评估',
